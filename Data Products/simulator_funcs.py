...
def get_routing_and_emissions(
    ...
    if routing_speed == None:
        ...
        if routing_speed == 0.0 or type(routing_speed) != float:
            log.warning(
                "Could not find max speed to use for routing speed, setting it to 10. If another value is wanted, specify routing_speed when calling the function."
            )
            routing_speed = 10
    ...
    if draught == None:
        ...
        if draught == 0.0:
            log.warning(
                """Could not find draught, setting it to 7. Another value can be cohse by specifying draught when calling the function."""
            )
            draught = 7
    ...
    df = simulate(
        ...
        resolution_minutes=time_resolution,
        cost_density=cost_density,
    )
...
def emissions_multiple_vessels(
    coordinates,
    df_input,
    dist_port,
    dist_shore,
    ...
    for mmsi in df_input.index:
        df, r = get_routing_and_emissions(
            mmsi,
            coords=coordinates,
            dist_port=dist_port,
