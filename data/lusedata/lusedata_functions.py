...
def make_engine():
    ...
    engine = create_engine(getenv("DATABASE_URL"),
                                     connect_args={"application_name": "hub_explorer",
                                         'sslmode': 'require'})
...
def average_lice_plots(year_range, week_range, lice_type, location_level, location, y_scale=None):
    ...
    if y_scale:
        chart = alt.Chart(df_plot_agg).mark_line().encode(
...
def plot_treatments(year_range, week_range, location_level, location, treatments, y_scale):
    ...
    for treatment_type in treatments:
        df_temp = df_plot_agg[["uke", "total_{}".format(treatment_type), "total_voksne_hunnlus", "beholdning"]]
    ...
    base = alt.Chart(df_pivot).encode(
        alt.X("uke", axis=alt.Axis(title="Uke"), scale=alt.Scale(domain=(week_range[0], week_range[1] + 1)))
    )

    selection = alt.selection_multi(fields=["type_behandling"], bind="legend")

    if y_scale > 0:
        treatments = base.mark_bar().encode(
            alt.Y("sum(antall)", title="Antall behandlinger", scale=alt.Scale(domain=[0, y_scale])),
            tooltip=[alt.Tooltip("sum(antall)", title='antall behandlinger')],
            color=alt.Color("type_behandling", title="Type behandling", scale=alt.Scale(scheme="paired")),
            opacity=alt.condition(selection, alt.value(1), alt.value(0.2)),
        ...
        treatments = base.mark_bar().encode(
            ...
            tooltip=[alt.Tooltip("sum(antall)", title='antall behandlinger')],
            color=alt.Color("type_behandling", title="Type behandlinger", scale=alt.Scale(scheme="paired")),
            opacity=alt.condition(selection, alt.value(1), alt.value(0.2)),
        ).add_selection(selection)
    ...
    chart = alt.layer(treatments, total).resolve_scale(
        y="independent"
